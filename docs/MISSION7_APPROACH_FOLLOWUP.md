...
## Route generalization analysis — September 22

The existing route traces were analyzed before launching another evaluation.
The key result is a controller mismatch: the exact 10/10 gate used
`PlateStage`, while `21398514` used `PlateSafeRouteController` with `switch`
and `plate_brake` states. The route traces contain **0/32 guarded-stage
activations**; therefore the route result cannot be interpreted as a failed
generalization test of the guarded maneuver. The read-only per-episode report
is [`route-failure-analysis/result.json`](../results/mission7-approach-followup-20260922/route-failure-analysis/result.json),
generated by [`mission7_route_failure_analysis.py`](../scripts/mission7_route_failure_analysis.py).

The ten successful guarded replay entries define the observed deterministic
entry distribution, not a certified basin of attraction:

| Entry feature | Observed range |
|---|---:|
| plate-center distance | 0.736–0.782 m |
| lateral offset | −0.782–+0.743 m |
| longitudinal offset | −0.692–+0.130 m |
| heading error | −0.035–1.623 rad |
| base speed | 0.105–0.805 m/s |
| tilt | 0.016–0.149 rad |

Route-level taxonomy from the existing 16+16 traces is:

| Stage | Before plate/navigation | Manipulation before plate | Plate interaction incomplete | After interaction/recovery | Success | Guarded activations |
|---|---:|---:|---:|---:|---:|---:|
| Doors | 8 | 0 | 3 | 4 | 1 | 0/16 |
| Transport | 4 | 4 | 2 | 6 | 0 | 0/16 |

The route episodes that did reach `switch` entered a different state
distribution: the measured plate distance ranged about 0.05–0.876 m, heading
error included values near π, and the world −x layout (validation index 8) had
heading error 3.123 rad with lateral offset −0.308 m. The route traces recorded
no wall/door/post contact before these falls; plate contact is available only as
the controller's `plate_contacts_seen` flag, so `plate_brake` time is reported
as an approximate interaction boundary rather than an exact contact timestamp.
Falls and timeouts therefore separate into navigation/acquisition failures and
post-plate failures; they are not all one plate-physics failure.

This supports the narrow-gate hypothesis and makes route-to-stage handoff the
dominant experimental gap, but not the sole route failure cause: many episodes
fail before any plate interaction. World −x is not statistically
disproportionate in this route sample (1/16 in each stage), although that one
layout is a high-heading-error outlier and the independent privileged Approach
matrix remains 8/16 in world −x.

The smallest discriminating intervention was a six-episode deterministic probe
on validation layouts 1, 4, and 8 in both Doors and Transport. It changed only
the existing `switch` handoff to the guarded `PlateStage`; route navigation,
geometry, activation semantics, terminal criteria, and the fall predicate were
unchanged. The first submission `21399179` failed before execution because the
node's bare Python lacked numpy. The corrected source-frozen submission
`21399211` completed on `cn-c22`. Compact evidence is
[`route-handoff-probe-summary.json`](../results/mission7-approach-followup-20260922/route-handoff-probe-summary.json);
the full raw traces remain under the v3 probe directory.

The probe recorded guarded-stage activation in 4/6 episodes, completion in
2/6, and one full success:

| Stage/layout | Outcome | Guarded stage | First meaningful divergence |
|---|---|---|---|
| Doors/1 | fall at 22.28 s | not activated | plate contact and `plate_brake` occurred before the `switch` handoff |
| Doors/4 | fall at 21.96 s | completed at 21.80 s | post-stage route continuation fell during `advance` |
| Doors/8 (world −x) | excessive collision | approach/settle only | world −x handoff did not complete |
| Transport/1 | fall at 22.88 s | not activated | same pre-handoff plate-brake failure as Doors/1 |
| Transport/4 | success | both crossings completed | no observed divergence; full transport succeeded |
| Transport/8 (world −x) | fall at 23.44 s | reached `cross`, incomplete | fell during guarded crossing |

This separates the mechanisms. Route-to-stage handoff is the dominant
experimental gap and is a real failure mode for layouts 1, but it is not the
sole cause: Doors/4 failed after a completed guarded crossing, Transport/4
succeeded from a comparable entry, and world −x layout 8 failed inside the
