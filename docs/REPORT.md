...
| **8** | **Only the 22-DoF model crosses the lab floor upright** — *single-rollout claim; a 5-seed rerun on 2026-09-23 (`results/lab-traverse-20260923/lab_traverse_5seed.json`) finds 22-DoF finishing 5/20 rollouts and 12-DoF 2/20, the 12-DoF terrain policy finishing 2/5, so the honest statement is 'the 22-DoF model clears the course more often, and neither clears it reliably'.* Three of four biped policies end at a **2.5 cm cable** (9% of leg length); two of four humanoid policies clear the whole course. The same recipe that falls at $x=1.66$ on 12 DoF finishes on 22 — though one arm is *worse* with arms. |
