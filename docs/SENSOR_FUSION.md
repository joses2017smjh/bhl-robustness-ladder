...
≈30 ms end-to-end, sensor to observation. The 12-DoF maze policies are stricter
still: trained with a zero-latency IMU, they drop from 0.98 to 0.31 with a single
20 ms IMU delay (SF-01), which is why SF-04 randomizes a 0–1 step delay in training. The IM10A's factory 10 Hz output (100 ms period) cannot
...
|---|---|---|---|---|
| SF-01 | How much IMU noise and delay do the trained Full-stage maze checkpoints tolerate? | **DONE** (`21402926`, `results/repo-gpu-20260923/maze-sf01/summary.json`): four Full s0 checkpoints × 14 settings, 32 envs each, corruption on | 0.6 GPU-h spent | Pooled first-episode success: baseline 0.977; gyro 0.10 → 0.984, 0.20 → 0.953; gravity 0.05 → 0.977, 0.10 → 0.859; **IMU delay 20 ms → 0.312, 40 ms → 0.023, 80 ms → 0.000**. Noise is tolerated; **any IMU latency is not** — the maze policies were trained with zero-latency IMU |
| SF-02 | How accurate must maze localization be? | **DONE both sides**: MuJoCo `21402726` (33 CPU episodes), Isaac `21402926` | done | MuJoCo route controller: bias ≤ 0.15 m 3/3, 0.20 m 1/3, noise 0.10 m 3/3, heading ≤ 20° 3/3, **drift 0.01 m/s 0/3**. Isaac teacher: any tested error (5–10 cm, 3–10°) costs ≈12 points (0.977 → 0.85), 20 cm → 0.69 — a step, not a slope, likely through the stop/dwell predicate. Requirement: bounded map-anchored error, no drift |
