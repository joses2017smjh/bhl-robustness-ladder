...
zero-latency IMU, keep 27/32 with one 20 ms step of IMU delay but fall to 0/32
with two (clean control, 2026-09-24; the earlier 0.31 figure was confounded by
leftover noise), which is why SF-04 randomizes a 0–1 step delay in training. The IM10A's factory 10 Hz output (100 ms period) cannot
...
|---|---|---|---|---|
| SF-01 | How much IMU noise and delay do the trained Full-stage maze checkpoints tolerate? | **RERUNNING** (`21404944`): the first sweep (`21402926`) was confounded — the probe left gyro std 0.20 / gravity std 0.10 in place from the noise settings, so its delay and localization rows are withdrawn; the pure-noise rows stand (gyro 0.10 → 0.984, 0.20 → 0.953; gravity 0.05 → 0.977, 0.10 → 0.859) | 0.6 GPU-h + rerun | Clean control so far (Both s0, training noise): baseline 30/32, **delay 1 step 27/32, delay 2 steps 0/32**, lidar off 18/32, stereo off 0/32. Pooled numbers from the rerun replace the withdrawn 0.312 |
| SF-02 | How accurate must maze localization be? | MuJoCo **DONE** (`21402726`); Isaac **RERUNNING** (`21404944`) | 33 CPU episodes + rerun | MuJoCo route controller: bias ≤ 0.15 m 3/3, 0.20 m 1/3, noise 0.10 m 3/3, heading ≤ 20° 3/3, **drift 0.01 m/s 0/3**. The Isaac 'flat ≈0.85 step' is withdrawn: it was the leftover gravity-0.10 noise, not the teacher error. Requirement stands: bounded map-anchored error, no drift |
...
| SF-04 | Does a recurrent, modality-dropout `both` policy degrade gracefully? | **NEGATIVE** (`21402927`/`21403140` Approach, `21403141` Corridor): BothRobust arm, recurrent actor, per-episode LiDAR/stereo dropout p 0.2, IMU bias 0.02, IMU delay 0–1 step | ≈2 GPU-h spent; Full and evaluation cancelled | Approach gate 31/32, but only because the button is within one stagger of the spawn; **Corridor 0/32** — 92 % of episodes fall from the first Corridor iteration and 100 % at the end (≈0.9 s). The combined recipe did not learn a stable gait in 2,000 iterations. **Follow-up running**, one job at a time from the working `Both` checkpoint: delay-only fine-tune `21404180` → teacher→recurrent-student distillation `21404181` → all-ingredients MLP fine-tune `21404182` → dropout-only `21404183` → bias-only `21404184`; each with the untouched `Both` control under the same probe matrix |
