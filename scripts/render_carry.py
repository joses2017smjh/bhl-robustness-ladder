...
class CarryRecorder(EpisodeRecorder):
    ...
    def capture_frame(self, data, lifts, pinched, t: int,
                      flash: str | None = None) -> np.ndarray:
        self.renderer.update_scene(data, camera=self.camera)
        frame = np.zeros((self.height, self.width, 3), dtype=np.uint8)
        frame[:self._scene_h] = self.renderer.render()
...
def main() -> None:
    ...
    rec = CarryRecorder(model, args.out, fps=1.0 / POLICY_DT,
                        ...
                        horizon=EPISODE_STEPS, width=1180, height=620,
                        caption=cap)
    rec._track_id = -1
