...
### Rough terrain — randomization gets you most of the way, terrain training gets the rest

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="results/charts/terrain_retention-dark.svg">
  <img alt="Fall rate vs terrain difficulty for three regimes: no randomization fails immediately, randomization only degrades past d=0.4, terrain-trained stays at zero." src="results/charts/terrain_retention-light.svg">
</picture>

Scored in MuJoCo, on ground generated by different code than the training
terrain. Three regimes, 3 seeds x 6 commands per point:
...
**The headline is that plain domain randomization — with zero terrain exposure —
carries the robot to roughly `d = 0.4` on its own.** A policy that has only ever
walked on a flat plane, but was trained with randomized friction, mass, and
actuator gains, handles moderately rough ground without ever having seen any.
Terrain training is what holds past that point: it is still at 0% falls where
the randomized-only policy has degraded to 39%.

> **Correction.** An earlier version of this README used the `s = 0` (no
> randomization) policy as the terrain baseline and reported that flat-trained
> policies "collapse between d = 0.05 and d = 0.10". That is true of *that*
> policy, but it was the most fragile one available and made terrain training
> look far more necessary than it is. Compared against the repo-default policy,
> terrain training buys nothing measurable below `d ≈ 0.4`. The weak-baseline
> version overstated the result.

The training-side curriculum tells a consistent story: it plateaus at about
...
- The obstacles were **not** the limiter. The arm *with* obstacles reaches a
  ...
  expected when flagging obstacles as the risky choice.
- **That ablation is confounded and should not be quoted as clean.** Removing
  ...
  average* rather than being "bumpy minus obstacles". A correct ablation holds
  the other proportions fixed and replaces the obstacle share with flat ground.

### Demo clips

`results/demo/` holds paired success/failure clips for both robustness
experiments, rendered by the same rollout that produced the numbers above, so
each clip is the episode a row describes.

| clip set | contrast | outcome |
|---|---|---|
| `push-adaptive__*` vs `nopush-baseline__*` | identical 0.5 m/s shoves | **0/6 falls** vs **3/6 falls** |
| `terrain-trained__*_d0.80` vs `flatDR-baseline__*_d0.80` | identical rough ground | **0/6 falls** vs **3/6 falls** |

Filenames end in `__OK` or `__FELL`, so clips self-sort into successes and
failures. Pushes flash an amber border, falls flash red and hold ~0.6 s so the
failure is readable rather than cutting the instant the robot tips.
